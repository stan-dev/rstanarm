...
sum-of-squared residuals is minimized, which occurrs when
...
is the standard deviation of the outcome, and $\frac{1}{N-1}$ is the standard
deviation of the $k$ column of $\mathbf{Q}$. Then let 
...
non-uniform prior on $R^2$ is somewhat analagous to the lasso approach, which is 
popular in machine learning and produces better out-of-sample predictions than
...
encourages $\boldsymbol{\beta} = \omega s_y \sqrt{N-1} \mathbf{R}^{-1} 
\boldsymbol{rho}$ to be closer to the origin.
...
which is proportional to a Jeffreys prior on the unkown $\sigma_y$, 
...
Most of the estimated shape parameters for the Generalized Pareto distribution 
...
--- and only has $2$ out of $24$ shape estimates in the "danger zone". We 
might want to reexamine these two observations (1, and 15)
```{r LOO_POST}
