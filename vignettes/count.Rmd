...
This vignette explains how to estimate generalized linear models (GLMs) for count data using the __rstanarm__
package.
...
Although other link functions are possible, the standard link function for a 
...
stan_glm2 <- update(stan_glm1, family = neg_binomial_2, 
                    prior_ops = prior_options(prior_scale_for_dispersion =
1))
```
