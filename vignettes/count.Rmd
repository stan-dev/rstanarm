...
```{r, ROACHES-negbin-posterior_predict}
...
with a simple histogram:

```{r, ROACHES-negbin-posterior_predict2, fig.width = 4}
qplot(T_yrep2, ylab = "", xlab = "Proportion of zeros") + 
  geom_vline(xintercept = T_y, color = "skyblue2", size = 2)
```

This histogram shows the distribution of the proportion of zeros across the 
datasets simulated under the negative binomial model. The value of the statistic
`T_y` (computed from the sample) is the vertical line, which falls nicely in the
center of the distribution and indicates that the the observed proportion of
zeros is quite plausible under this model.
