---
title: "Estimating Generalized Linear Models for Continuous Data with the rstanarm Package"
...
%\VignetteIndexEntry{Estimating Generalized Linear Models for Continuous Data with the rstanarm Package}
-->
...
binary/binomial and count data.
...
Package", although this vignette does also give a few examples of model checking
...
The `stan_lm` function (which has its own vignette) fits regularized linear 
models using a novel means of specifying priors for the regression coefficients.
Here we focus on the `stan_glm` function, which simply puts independent priors
...
which are currently `normal(0,10)` for the intercept and `normal(0,5)` for the
other regression coefficients. For an overview of the many other available prior
distributions see `help("prior", package = "rstanarm")`.
...
post1 <- stan_glm(kid_score ~ mom_hs, data = kid_iq, family = gaussian(link = "identity"), 
                  chains = CHAINS, cores = CORES, seed = SEED, iter = 300)
...
base + 
  geom_abline(data = draws, aes(intercept = a, slope = b), 
              color = "skyblue", size = 0.1, alpha = 0.25) + 
  geom_abline(intercept = coef(post1)[1], slope = coef(post1)[2], 
              color = "skyblue4", size = 0.3)
```
...
```{r, KIDIQ-plot3}
regline0 <- function(x, ests) cbind(1, 0, x) %*% ests 
regline1 <- function(x, ests) cbind(1, 1, x) %*% ests

args <- list(ests = coef(post3))
ggplot(kid_iq, aes(x = mom_iq, color = factor(mom_hs, labels = c("No HS", "HS")))) + 
  geom_point(aes(y = kid_score)) +
  stat_function(fun = regline0, args = args, aes(color = "No HS"), size = 1.5) + 
  stat_function(fun = regline1, args = args, aes(color = "HS"), size = 1.5)
```

```{r, KIDIQ-plot4}
regline0 <- function(x, ests) cbind(1, 0, x, 0 * x) %*% ests 
regline1 <- function(x, ests) cbind(1, 1, x, 1 * x) %*% ests
...
The `ppcheck` function generates a variety of plots comparing the observed 
...
`help("ppcheck", package = "rstanarm")` has details on all of the `ppcheck`
options.

First we'll look at a plot directly comparing the distributions of $y$ and
$y^{\rm rep}$. The following call to `ppcheck` will create a plot juxtaposing
the histogram of $y$ and histograms of five $y^{\rm rep}$ datasets:

```{r, KIDIQ-ppcheck1}
ppcheck(post4, check = "distributions", overlay = FALSE, nreps = 5)
```
...
Another useful plot we can make using `ppcheck` shows the distribution of a test
quantity $T(y^{\rm rep})$ compared to $T(y)$, the value of the quantity in the 
observed data. When the argument `check = "test"` is specified, `ppcheck` will 
...
```{r, KIDIQ-ppcheck2}
ppcheck(post4, check = "test", test = "mean")
```

We can also specify two test quantities and look at a scatterplot:

```{r, KIDIQ-ppcheck3}
ppcheck(post4, check = "test", test = c("mean", "sd"))
```
...
```{r, KIDIQ-validation, eval=FALSE, include=FALSE}
# # External Validation
# source(paste0(ROOT, "ARM/Ch.3/kids_before1987.data.R"), 
#        local = kid_iq, verbose = FALSE)
# source(paste0(ROOT, "ARM/Ch.3/kids_after1987.data.R"), 
#        local = kid_iq, verbose = FALSE)
# post5 <- stan_lm(ppvt ~ hs + afqt, data = kid_iq,
#                  prior = R2(location = 0.25, what = "mean"), seed = SEED)
# y_ev <- posterior_predict(post5, newdata = 
#                           data.frame(hs = kid_iq$hs_ev, afqt = kid_iq$afqt_ev))
# par(mfrow = c(1,1))
# hist(-sweep(y_ev, 2, STATS = kid_iq$ppvt_ev, FUN = "-"), prob = TRUE,
