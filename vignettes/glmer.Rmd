...
This vignette explains how to use the `stan_lmer`, `stan_glmer`, and `stan_nlmer` 
functions in the __rstanarm__ package to estimate linear and generalized (non-)linear 
models with parameters that may vary across groups. Before continuing, we
...
$$\mathbf{y} \thicksim \mathcal{N}\left(\alpha + \mathbf{X}\boldsymbol{\beta}, \sigma^2 \mathbf{Z}^\top \boldsymbol{\Sigma} \mathbf{Z} \right),$$
...
post3 <- stan_nlmer(conc ~ SSfol(Dose, Time,lKe, lKa, lCl) ~
                    (0 + lKa + lCl | Subject), data = Theoph,
                    chains = CHAINS, cores = CORES, seed = SEED, 
                    init_r = 0.5)
```
which is increasing in the dosage and decreasing in time
```{r}
nd <- expand.grid(Dose = 3:6, Time = 1:25)
nd$Subject <- factor("0", levels = 0:12) # necessary, even with re.form = NA
PPD <- posterior_predict(post3, newdata = nd, re.form = NA)
PPD_df <- data.frame(Dose = nd$Dose, each = nrow(PPD),
                     Time = nd$Time, each = nrow(PPD),
                     conc = colMeans(PPD))
ggplot(PPD_df, aes(x = Dose, y = Time, z = conc)) + 
