...
it is necessary to specify group-specific terms as a (list of) formula(s) that
...
library(rstanarm)
data(roaches)
roaches$off <- log(roaches$exposure2)
roaches$treatment <- as.factor(roaches$treatment)
post <- stan_gamm4(y ~ s(roach1, by = treatment) + offset(off), 
                   random = ~(1 | senior),
                   data = roaches, family = poisson, QR = TRUE,
                   chains = CHAINS, cores = CORES, seed = SEED)
```
```{r}
do.call(gridExtra::grid.arrange, args = gg_nlf(post))
```

Here we see that the relationship is not estimated to be particularly
stable with only $262$ total observations. When treatment is one,
the non-linear function seems reasonable, staying steady near one
until a sufficient number of previous roaches becomes sufficiently
large and then it takes off before perhaps decreasing if the building
becomes overly saturated with roaches. When treatment is zero, the
non-linear function has an odd dip between the stable range and the
increasing range. So, in this case, it would probably be better to
estimate the model without allowing the non-linear functions to 
vary by treatment category.
