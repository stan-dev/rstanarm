...
`bayes_interval` function to get Bayesian uncertainty intervals. The uncertainty
intervals are computed by finding the relevant quantiles of the draws from the
posterior distribution. For example, to compute 50% intervals we use:

```{r, ARSENIC-CI}
round(bayes_interval(fit1, prob = 0.5), 2)
```

For more on `bayes_interval` and interpreting the parameter estimates from a Bayesian
