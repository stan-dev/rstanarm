...
knitr::opts_chunk$set(dev = 'svg', fig.align='center', fig.height = 3, message = FALSE,
                      warning = FALSE) 
...
the product of conditionally independent binomial distributions.
...
[logit](https://en.wikipedia.org/wiki/Logit) and [probit](https://en.wikipedia.org/wiki/Probit)
...
link, we recommend the logit simply because it will be slightly faster.

In theory there are infinitely many possible link functions, although in
practice only a few are typically used. Other common choices are the cauchit and
cloglog functions, which can also be used with `stan_glm` (every link function
compatible with`glm` will work with `stan_glm`).
...
After reading in the data from the `'wells.csv'` file, we first rescale the
...
```{r, ARSENIC-plot-dist100, fig.height=2}
ggplot(wells, aes(x = dist100, y = ..density.., color = switch)) + 
  geom_histogram(data = subset(wells, switch == 1), alpha = 0.3) + 
  geom_histogram(data = subset(wells, switch == 0), alpha = 0.3)
```

In the plot above the bars outlined in light blue correspond to the 
...
can be estimated using the `stan_glm` function:
...
arguments `cores` (how many cores we want the computer to utilize) and `seed` 
...
(To look at the uncertainty in our estimates we can use the `confint` function
to get Bayesian credible intervals. For an example of using `confint` and more 
information about interpreting the parameter estimates from a Bayesian model
see __Step 2__ in the "How to Use the __rstanarm__ Package".)
...
lower predicted probabilities of switching. At the extreme ($\approx 300$ meters), 
...
We can use the same call to `stan_glm` as before but adding the additional 
predictor `arsenic` to the formula. Just like with `glm` we can use the 
`update` function with `stan_glm`:

```{r ARSENIC-stan_glm2}
fit2 <- update(fit1, formula = switch ~ dist100 + arsenic) 
# (note: we could also use formula = '. ~ . + arsenic' instead of rewriting the formula)
...
ggplot(data = grid, aes(x = dist100, y = arsenic)) + 
...
# Quantiles
qx <- quantile(wells$dist100, seq(0, 1, 0.25))
qy <- quantile(wells$arsenic, seq(0, 1, 0.25))  
# Function to generate the calls to stat_function
pred_fn <- function(model, i, q, x_or_y) {
  args <- list(coef = coef(model))
  args[[x_or_y]] <- q[i]
  stat_function(fun = pr_switch2, color = "gray35", args = args)
}
...
for (i in 1:5) {
  vary_dist <- vary_dist + pred_fn(fit2, i, qy, "y")
  vary_arsenic <- vary_arsenic + pred_fn(fit2, i, qx, "x")
}
gridExtra::grid.arrange(vary_dist, vary_arsenic, ncol = 2)
```
...
The vignette for `stan_lm` also has an example of using the __loo__ function
