...
```{r, ARSENIC-plot-dist100, fig.height=3}
...
To get a sense for the uncertainty in our estimates we can use the `prob_int` 
function to get Bayesian uncertainty intervals. The uncertainty intervals are 
computed by finding the relevant quantiles of the draws from the posterior 
distribution. For example, to compute 50% intervals we use:

```{r, ARSENIC-CI}
round(prob_int(fit1, prob = 0.5), 2)
```

For more on `prob_int` and interpreting the parameter estimates from a Bayesian
model see Step 2 in the "How to Use the __rstanarm__ Package" vignette.
...
The vignette for the `stan_lm` function also has an example of using the `loo`
