...
Dirichlet prior on $\Pr\left(y=j\right)\left.\right|\overline{\mathbf{x}}$,
...
$\pi_j = \Pr\left(y=j\right)\left.\right|\overline{\mathbf{x}}$.
...
```{r, cutpoints, echo=FALSE}
zeta_medians <- round(summary(post0, pars = "zeta")[, "50%"], 1)
```
...
are $\zeta_1 = `r zeta_medians[1]`$, $\zeta_2 = `r zeta_medians[2]`$, 
and $\zeta_3 = `r zeta_medians[3]`$, which cut 
...
values of the `shape` and `rate` arguments, which implies a "scobit"-type 
likelihood where the probability of success is given by 
$F\left(\eta\right)^\lambda$, where $F\left(\right)$ is some CDF and 
$\lambda > 0$ is a skewing parameter that has a gamma prior with a given 
`shape` and `rate`. If $\lambda \neq 1$, then the relationship between $\eta$ 
and the probability of success is asymmetric. In principle, it seems appropriate
to estimate $\lambda$ but in practice, a lot of data is needed to estimate
$\lambda$ with adequate precision. In the previous example, if we specify
`shape = 2` and `rate = 2` to reflect our prior beliefs that $\lambda$ is
expected to be $1$ but has a variance of $\frac{1}{2}$, then the loo calculation
yields many Pareto shape parameters that are excessively large. However, with
more than $189$ observations, such a model may be more fruitful.
