...
fit_nopool <- update(fit_pool, formula = . ~ 0 + Player, 
                     prior = normal(-1, 2.5))
...
fit_partialpool <- stan_glmer(cbind(Hits, AB - Hits) ~ (1 | Player), 
                              prior_intercept = normal(-1, 2.5),
                              data = bball, family = binomial("logit"))
```
...
thetas <- sweep(draws[, -1], MARGIN = 1, STATS = draws[, 1], "+")
...
```{r, plot-compare-models, fig.width = 8, fig.height = 4}
library(ggplot2)
