...
(MAD) from the posterior medians. In addition, using the `prob_int` function
we can obtain a Bayesian uncertainty interval for $\beta_1$:
```{r CI}
ci95 <- prob_int(womensrole_bglm_1, pars = "education", prob = 0.95)
round(ci95, 2)
```
...
__rstanarm__ default) or approximate Bayesian inference the `prob_int` function
