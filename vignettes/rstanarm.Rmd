...
agree with the statement that have the same level of education and the same
...
the models themselves). The references at the end of each subsection below provide
recommendations for where to find more information on the relevant issues. 
...
bad_rhat <- stan_glmer(mpg ~ wt + (1|cyl) + (1|gear), data = mtcars, 
                       iter = 100, chains = CHAINS, cores = CORES, seed = SEED)
good_rhat <- update(bad_rhat, iter = 2000, # the default for iter
                    chains = CHAINS, cores = CORES, seed = SEED)
```
...
manual, Version 2.8.0_. http://mc-stan.org/documentation. See the 
'Markov Chain Monte Carlo Sampling' chapter. 
...
case the posterior sample may be biased. Lowering the step size can help reduce
the number of divergent transitions. The recommended method is to increase the 
`adapt_delta` parameter -- target rate at which proposals are accepted -- which 
will in turn reduce the step size. To change the default value of `adapt_delta`
(__rstanarm__ uses a default of 0.95) you can use the `control` argument, which 
should be a named list. For example, to increase `adapt_delta` to 0.99 (a good 
value to try first) you would pass the argument `control = list(adapt_delta = 0.99)`
to `stan_lm` (or any the other __rstanarm__ modeling functions):


```{r, Divergent-fit, results='hide', warning=TRUE}
divergences <- stan_lm(mpg ~ ., data = mtcars, prior = R2(0.75), 
                       iter = 500, chains = CHAINS, 
                       cores = CORES, seed = SEED)
no_divergences <- update(divergences, control = list(adapt_delta = 0.99))
```
...
Intuitively, this is because smaller step sizes means that more steps are
...
involves putting a cap on the depth of the trees that it evaluates during each 
iteration. This is controlled through a maximum depth parameter `max_treedepth`. 
When the maximum allowed tree depth is reached it indicates that NUTS is 
terminating prematurely to avoid excessively long execution time. If __rstanarm__
arm prints a warning about transitions exceeding the maximum treedepth you 
should try increasing the `max_treedepth` parameter using the `control` argument.
For example, to increase `max_treedepth` to 20 (the default used __rstanarm__ 
is 15) you can provide the argument `control = list(max_treedepth = 20)`
to any of the __rstanarm__ modeling functions. 
