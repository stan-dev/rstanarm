...
library(rstanarm)
fit1 <- stan_betareg(y ~ x | z, data = dat, link = "logit", link.phi = "log", iter = 2000, cores = 4, algorithm = "sampling")
fit2 <- stan_betareg(y ~ -1 + x , data = dat, link = "logit", link.phi = "log", iter = 2000, cores = 4, algorithm = "sampling")
round(coef(fit1), 2)
round(coef(fit2), 2)
```
...
gas_fit1 <- stan_betareg(yield ~ temp + batch, data = GasolineYield, link = "logit", iter = 1000, cores = 4, algorithm = "sampling")
gas_fit2 <- stan_betareg(yield ~ temp | pressure, data = GasolineYield, link = "logit", link.phi = "log", iter = 1000, cores = 4, algorithm = "sampling")
...
library(gridExtra)
gas_plot1 <- pp_check(gas_fit1) + ggtitle("gas_fit1") + xlim(0, 1)
gas_plot2 <- pp_check(gas_fit2) + ggtitle("gas_fit2") + xlim(0, 1)
