...
```{r simulated-data, fig.width=6}
dat <- list()
dat$eta <- c(1, -0.2)
dat$gamma <- c(1.8, 0.4)
dat$N <- 500
dat$x <- rnorm(dat$N, 2, 2)
dat$z <- rnorm(dat$N, 2, 2)
dat$mu <- binomial(link = logit)$linkinv(dat$eta[1] + dat$eta[2]*dat$x)
dat$phi <- binomial(link = log)$linkinv(dat$gamma[1] + dat$gamma[2]*dat$z)
dat$y <- rbeta(dat$N, dat$mu * dat$phi, (1 - dat$mu) * dat$phi)
dat <- data.frame(cbind(dat$y, dat$x, dat$z))
...
```{r simulated-fit, results = "hide", cache=TRUE}
library(rstanarm)
fit1 <- stan_betareg(y ~ x | z, data = dat, link = "logit", link.phi = "log", algorithm = "sampling")
fit2 <- stan_betareg(y ~ x, data = dat, link = "logit", link.phi = "log", algorithm = "sampling")
round(coef(fit1), 2)
round(coef(fit2), 2)
```
...
library(gridExtra)
plot1 <- pp_check(fit1) + ggtitle("True Model: y ~ x | z")
plot2 <- pp_check(fit2) + ggtitle("False Model: y ~ x")
...
gas_fit1 <- stan_betareg(yield ~ temp + batch, data = GasolineYield, link = "logit", algorithm = "sampling")
gas_fit2 <- stan_betareg(yield ~ temp | pressure, data = GasolineYield, link = "logit", algorithm = "sampling")
...
library(gridExtra)
gas_plot1 <- pp_check(gas_fit1) + ggtitle("gas_fit1")
gas_plot2 <- pp_check(gas_fit2) + ggtitle("gas_fit2")
...
Note that the `sqrt` link function (applied to the second set of regressors) is known to be unstable, so it
is advised to specify a different link function or to model the dispersion parameter as a constant. 
