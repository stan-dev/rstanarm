  ...
  \propto
    \log \Bigg[
      \Bigg(
        \prod_{m=1}^M 
          \prod_{j=1}^{n_i} 
            p(y_{ijm}(t) | \boldsymbol{b}_{i}, \boldsymbol{\theta})
      \Bigg)
...
\log p(\boldsymbol{\theta}, \boldsymbol{b}_{i} | \boldsymbol{y}_{i}, T_i, d_i)
  \propto
    \Bigg(
      \sum_{m=1}^M 
        \sum_{j=1}^{n_i} 
          \log p(y_{ijm}(t) | \boldsymbol{b}_{i}, \boldsymbol{\theta})
    \Bigg) +
...
## Posterior predictions for in-sample individuals
...
$$ 
p(y^{*}_{im}(t) | \mathcal{D}) = 
  \int 
    \int 
      p(y^{*}_{im}(t) | \boldsymbol{\theta}, \boldsymbol{b}_i)
      p(\boldsymbol{\theta}, \boldsymbol{b}_i | \mathcal{D}) 
...
p(S^{*}_{i}(t) | \mathcal{D}) = 
  \int
    \int
      p(S^{*}_i(t) | \boldsymbol{\theta}, \boldsymbol{b}_i)
      p(\boldsymbol{\theta}, \boldsymbol{b}_i | \mathcal{D})
    \space d \boldsymbol{b}_i
  \space d \boldsymbol{\theta}      
$$

Note that for simplicity we have ignored the implicit conditioning on covariates; $\boldsymbol{x}_{im}(t)$ and $\boldsymbol{z}_{im}(t)$, for $m = 1,...,M$, and $\boldsymbol{w}_{i}(t)$. Since individual $i$ is included in the training data, it is easy for us to approximate these posterior predictive distributions by drawing from $p(y^{*}_{im}(t) | \boldsymbol{\theta}^{(l)}, \boldsymbol{b}_i^{(l)})$ and $p(S^{*}_i(t) | \boldsymbol{\theta}^{(l)}, \boldsymbol{b}_i^{(l)})$ where $\boldsymbol{\theta}^{(l)}$ and $\boldsymbol{b}_i^{(l)}$ are the $l^{th}$ $(l = 1,...,L)$ MCMC draws from the joint posterior distribution $p(\boldsymbol{\theta}, \boldsymbol{b}_i | \mathcal{D})$. 

These draws from the posterior predictive distributions can be useful for assessing the fit of the model. For example, 

- the draws from $p(y^{*}_{im}(t) | \mathcal{D})$ for $0 \leq t \leq T_i$ can be used to evaluate the fit of the longitudinal trajectory for the $m^{th}$ biomarker for the $i^{th}$ individual, and

- the draws from $p(S^{*}_{i}(t) | \mathcal{D})$ for $0 \leq t \leq T_{max}$ can be averaged across the $N$ individuals to obtain a standardised survival curve (discussed in greater detail in later sections), which can be compared to the observed survival curve (for example, the Kaplan-Meier)
...
$$
p(y^{*}_{im}(t) | \mathcal{D}, t > C_i) = 
  \int 
    \int 
      p(y^{*}_{im}(t) | \boldsymbol{\theta}, \boldsymbol{b}_i, t > C_i)
      p(\boldsymbol{\theta}, \boldsymbol{b}_i | \mathcal{D}) 
    \space d \boldsymbol{b}_i
  \space d \boldsymbol{\theta} 
$$ 
...
$$ 
p(S^{*}_{i}(t) | \mathcal{D}, t > C_i, T_i^* > C_i) 
  = 
  \frac
    {p(S^{*}_{i}(t) | \mathcal{D})}
    {p(S^{*}_{i}(C_i) | \mathcal{D})}
  = 
  \int
    \int
      \frac
        {p(S^{*}_i(t) | \boldsymbol{\theta}, \boldsymbol{b}_i)}
        {p(S^{*}_i(C_i) | \boldsymbol{\theta}, \boldsymbol{b}_i)}
      p(\boldsymbol{\theta}, \boldsymbol{b}_i | \mathcal{D})
    \space d \boldsymbol{b}_i
  \space d \boldsymbol{\theta}  
$$
...
- the draws from $p(y^{*}_{im}(t) | \mathcal{D}, t > C_i)$ can be plotted over the range $C_i \leq t \leq T_{max}$ to show the forecasted longitudinal trajectory for the $m^{th}$ biomarker for individual $i$, and

- the draws from $p(S^{*}_{i}(t) | \mathcal{D}, t > C_i, T_i^* > C_i))$ can be plotted over the range $C_i \leq t \leq T_{max}$ to show the estimated conditional probability of individual $i$ remaining event-free, given their last known survival time.
...
$$ 
p(y^{pred}(t) | \mathbf{x}_i, \boldsymbol{\theta}, \mathbf{b}_i, \mathcal{D}) = p(y^{pred} | \boldsymbol{\theta}, \mathbf{b}_i) p(\boldsymbol{\theta}, \mathbf{b}_i | \mathcal{D})
$$

$$ 
p(S_i^p (t) | \mathbf{x}_i^p (t), \mathbf{w}_i^p (t), \mathcal{D}) = p(S_i^p (t) | \mathbf{x}_i(t), \mathbf{w}_i(t), \boldsymbol{\theta}, \mathbf{b}_i) p(\boldsymbol{\theta}, \mathbf{b}_i | \mathcal{D})
$$
