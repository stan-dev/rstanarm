...
The output tells us that for each one unit increase in an individual's underlying level of log serum bilirubin, their estimated log hazard of death increases by `r round(as.numeric(coef(mod1)$Event[attributes(coef(mod1)$Event)$names == "Assoc|Long1|etavalue"]) - 1, 3)*100`% (equivalent to a `r round(exp(as.numeric(coef(mod1)$Event[attributes(coef(mod1)$Event)$names == "Assoc|Long1|etavalue"])), 2)`-fold increase in the hazard of death). The mean absolute deviation (MAD) is provided as a more robust estimate of the standard deviation of the posterior distribution. In this case the MAD_SD for the association parameter is `r round(as.numeric(mod1$ses$Event[attributes(mod1$ses$Event)$names == "Assoc|Long1|etavalue"]), 3)`, indicating there is quite large uncertainty around the estimated association between log serum bilirubin and risk of death (recall this is a small dataset containing only 40 patients!).
