...
### Univariate joint model with a current value association structure
...
#### Univariate joint model with a current value and current slope association structure
...
### Multivariate joint model with a current value association structure
...
### Posterior predictions for the marginal longitudinal trajectory

Now let's suppose we wanted to predict the longitudinal outcome for each of the biomarkers, marginalising over the distribution of the individual-specific parameters. To do this, we can pass a new data frame with the prediction data contained within it. Here, it will only require the variables `id` and `year`, since we don't have any other covariates in the longitudinal submodels. However, if we did have covariates in the longitudinal submodels, then our new data frame would also need to include values for those covariates. Here is the definition of the prediction data frame:
...
Because the individual with an ID `"new1"` was not used to estimate the model, the predictions be marginalised over the distribution of the individual-level random effects (we could have given the individual any subject ID, just as long as that subject ID didn't match any individual used in estimating the model). To predict the marginal longitudinal trajectory for each biomarker and plot it we can type:

```{r plot_traj_2183921}
p1 <- posterior_traj(mod3, m = 1, newdata = ndL)
p2 <- posterior_traj(mod3, m = 2, newdata = ndL)
plot(p1) + ggplot2::ggtitle("Log serum bilirubin")
plot(p2) + ggplot2::ggtitle("Serum albumin")
```

Because we are marginalising over the distribution of the individual-level random effects, we are incorporating all the variation related to between-individual differences, and therefore the prediction interval is wide (shown by the shaded area around the marginal longitudinal trajectory).

Now let's suppose we wanted to obtain predictions for the longitudinal outcome for a given covariate profile. Here, we will first generate the prediction data we wish to use. We will create prediction data for each possible covariate profile.

```{r newdata1}
ndL <- data.frame(
  id = c("male_notrt", "male_trt", "female_notrt", "female_trt"),
...
We can then pass this to the `posterior_traj` function to get the predicted longitudinal trajectory for each of these covariate profiles. We only included time zero (i.e. `year = 0`) for each individual in the prediction data frame, because we can use the `extrapolate = TRUE` and `control` arguments for to tell `posterior_traj` to extrapolate forward a specified distance. Alternatively we could have just included values for `year` in the prediction data frame according to the times we want to predict the longitudinal outcome at. Also note that our model included individual-specific parameters (i.e. random effects) and these individuals were not included in the estimation of the model, so we will be marginalising over the random effects distribution when we generate these predictions.

```{r pplong1}
p1 <- posterior_traj(mod3, m = 1, newdata = ndL, 
                     extrapolate = TRUE, control = list(edist = 10))
p2 <- posterior_traj(mod3, m = 2, newdata = ndL, 
                     extrapolate = TRUE, control = list(edist = 10))
plot(p1) + ggplot2::ggtitle("Log serum bilirubin")
plot(p2) + ggplot2::ggtitle("Serum albumin")
...
for a given individuals who were not in the dataset used to estimate the model (i.e. we want out-of-sample preditions). To demonstrate this, we will create a new data frame that includes two of the individuals from the PBC dataset, but we will rename their ID variable! This way, when we call the prediction function(s), they will not know that these were the same individuals used in estimating the model.
\
```{r newdata1}
ndL <- pbcLong[pbcLong$id %in% c(4,5), ]
ndS <- pbcLong[pbcSurv$id %in% c(4,5), ]
ndL$id <- paste0("new", ndL$id)
ndS$id <- paste0("new", ndS$id)
```

First we will predict the longitudinal trajectory

We can now call the posterior prediction functions (as demonstrated in the previous example), but this time passing the new prediction data containing the covariate information to the relevant `newdata` argument, as follows:
\
```{r plots, fig.width=6.5, fig.height=6}
p4 <- posterior_traj(mod3, m = 1, newdata = ndL)
p5 <- posterior_traj(mod3, m = 2, newdata = ndL)
p6 <- posterior_survfit(mod3, ids = 6:8, draws = 200)
pp4 <- plot(p4, plot_observed = TRUE, vline = TRUE)
pp5 <- plot(p5, plot_observed = TRUE, vline = TRUE)
plot_stack_jm(yplot = list(pp1, pp2), survplot = plot(p6))
```
