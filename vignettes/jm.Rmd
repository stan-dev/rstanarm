...
## Individual-specific predictions (in-sample individuals)
...
These draws from the posterior predictive distributions can be useful for assessing the fit of the model. For example, 
...
- the draws from $p(S^{*}_{i}(t) \mid \mathcal{D})$ for $0 \leq t \leq T_{max}$ can be averaged across the $N$ individuals to obtain a standardised survival curve (discussed in greater detail in later sections), which can be compared to the observed survival curve (for example, the Kaplan-Meier)
    
However, given that we have available an event or censoring time for each individual in our training data, it may make more sense to consider the following. For an individual $i$, who was in our training data, and who was known to be event-free up until their censoring time $C_i$, we wish to know the posterior predictive distribution for their longitudinal outcome at times $t > C_i$, that is 
...
and the posterior predictive distribution for their survival probability at times $t > C_i$ given that they were known to be event-free at least up until their censoring time $C_i$. This leads to the following conditional survival probability
...
It is useful to use draws from these conditional posterior predictive distributions to extrapolate into the future for individual $i$, using their longitudinal biomarker data collected between baseline and the last time they were known to be event-free (i.e. the data they contributed to the training data used for the model estimation). For example, we may wish to 

- the draws from $p(y^{*}_{im}(t) \mid \mathcal{D}, t > C_i)$ can be plotted over the range $C_i \leq t \leq T_{max}$ to show the forecasted longitudinal trajectory for the $m^{th}$ biomarker for individual $i$, and

- the draws from $p(S^{*}_{i}(t) \mid \mathcal{D}, t > C_i, T_i^* > C_i))$ can be plotted over the range $C_i \leq t \leq T_{max}$ to show the estimated conditional probability of individual $i$ remaining event-free, given their last known survival time.

## Individual-specific predictions (out-of-sample individuals)
...
1) The first way is to "marginalise" over the distribution of the individual-specific parameters. We wish to generate a predicted value for the $m^{th}$ longitudinal biomarker at time $t$ for a new individual $k$ for whom we do not have any observed data. We will denote this prediction $y^*_{km}(t)$ and note that it can be generated from the posterior predictive distribution
...
We can easily obtain draws for $\boldsymbol{\tilde{b}}_k$ in the same manner as for the individual-specific parameters $\boldsymbol{b}_i$. However, since individual $k$ did not provide any contribution to the training data, the posterior distribution for $\boldsymbol{\tilde{b}}_k$, that is, $p \Big( \boldsymbol{\tilde{b}}_k \mid \boldsymbol{\theta} \Big)$ is not conditional on the training data $\mathcal{D}$ but only on the hyperparameters $\boldsymbol{\theta}$. Therefore, we are effectively taking random draws from the posterior distribution for the individual-specific parameters^[note that draws from the posterior distribution of the group-specific parameters, i.e. MCMC draws of $\boldsymbol{\tilde{b}}_k$, are stored each time a joint model is estimated using `stan_jm` or any other __rstanarm__ modelling function; they are saved under the group-level ID `_NEW_`]. The predictions under this model can be thought of as marginal predictions, since they are marginalising over the distribution of the group-specific parameters unconditional on the training data. However, this mean they incoporate the uncertainty associated with the between-individual variation in the data, since we are effectively predicting for a new individual whom we have no information about (beyond the knowledge that they are drawn from the same population as the $i=1,...,N$ individuals in the training data). Therefore our uncertainty intervals (for example, 95% credible intervals or prediction intervals) are likely to be very wide.
 
The second way is to effectively ignore the group-level structure in the model. That is, to only predict with only the population-level parameters contributing to the model. For example, under a identity link function and normal error distribution (i.e. linear mixed effect longitudinal submodel), we would obtain draws from the distribution $y^{(l)}_{km}(t) \sim N \Big( \boldsymbol{x}^T_{km}(t) \boldsymbol{\beta}_m^{(l)}, \sigma_m^{(l)} \Big)$ where $\boldsymbol{\beta}_m^{(l)}$ and $\sigma_m^{(l)}$ are the population-level parameters and residual error standard deviation, respectively, for the $l^{th}$ draw of the MCMC samples. However, referring to this as a "marginal" prediction is somewhat misleading; although we are not conditioning on the individual-specific parameters explicitly, we are implicitly assuming we know that they are equal to zero with absolute certainty. That is, we are actually drawing from the posterior predictive distribution
...
The predicted survival probability for an individual $i$ can be defined as 

$$ 
p(y^{pred}(t) \mid \mathbf{x}_i, \boldsymbol{\theta}, \mathbf{b}_i, \mathcal{D}) = p(y^{pred} \mid \boldsymbol{\theta}, \mathbf{b}_i) p(\boldsymbol{\theta}, \mathbf{b}_i \mid \mathcal{D})
$$

$$ 
p(S_i^p (t) \mid \mathbf{x}_i^p (t), \mathbf{w}_i^p (t), \mathcal{D}) = p(S_i^p (t) \mid \mathbf{x}_i(t), \mathbf{w}_i(t), \boldsymbol{\theta}, \mathbf{b}_i) p(\boldsymbol{\theta}, \mathbf{b}_i \mid \mathcal{D})
$$

where $\mathbf{x}_i(t), \mathbf{w}_i(t)$ are the covariate values to predict at for individual $i$, $\boldsymbol{\theta} = \left( \boldsymbol{\beta}, \boldsymbol{\gamma}, \boldsymbol{\alpha} \right)$ is the collection of parameters under the joint model, and $\mathcal{D}$ is the observed sample data. 
...
$$ 
\hat{S}(t) = \frac{\sum_{i=1}^{N^{pred}} \hat{S}_i(t)}{N^{pred}}
$$

where $\hat{S}_i(t)$ is the survival probability for individual $i$ ($i = 1,...,N^{pred}$ at time $t$, and $N^{pred}$ is the number of individuals included in the prediction data. Of course, in reality, the predicted individual-specific survival probability is conditional on an a number of different things, namely: the individual's covariate values, the estimates of the population-level parameters, and the estimates of their individual-specific parameters, , i.e. $\hat{S}_i(t) = \hat{S}_i(t; \mathbf{x}_i(t), \mathbf{w}_i(t), \boldsymbol{\beta}, \hat{\boldsymbol{\gamma}}, \mathbf{b}_i)$, but we have left that conditioning out of the definition above for brevity. 

In effect, this marginalises (or standardises, depending on your preferred terminology) over the distribution of the covariates for the $N^{pred}$ individuals used in the predictions, and also marginalises over the distribution of individual-specific parameters for the $N^{pred}$ individuals used in the predictions. With a large random sample (for example, all individuals used in the estimation dataset) this may provide a good estimate of standardised survival probability; that is, it is marginalised over the observed distribution of covariates, and it is marginalised over the distribution of individual-specific parameters. 

Note however, that if the number of individual's included in the prediction data is small (e.g. we pass new data with just 2 individuals, say) then marginalising over their covariate distribtion may not be meaningful and, similarly, their joint random effects distribution may be a poor representation of the random effects distribution for the entire population. 
...
### Predicted individual-specific longitudinal trajectory (in-sample individuals)
...
### Predicted individual-specific survival curves (in-sample individuals)
...
\
```{r plots_23812}
p5 <- posterior_survfit(mod3, ids = 6:8, draws = 200)
pp5 <- plot(p5, vline = TRUE)
pp5
```
...
```{r plots_987321, eval=FALSE, fig.height=13}
...
### Predicted marginal longitudinal trajectory

Suppose we wanted to predict the longitudinal trajectory for each of the biomarkers, marginalising over the distribution of the individual-specific parameters. To do this, we can pass a new data frame with the covariate values we want to use in the predictions. Here, we will demonstrate this by obtaining the predicted trajectory for log serum bilirubin, under the multivariate joint model that was estimated previously. Our prediction data will require the variables `year`, `sex` and `trt`, since these were the covariates used in the longitudinal submodel. Moreover, we need to include the `id` variable in our prediction data because this is also relevant to the longitudinal submodel. However, since we want to marginalise over the individual-specific parameters (i.e. individual-level random effects) we need to use an ID in our prediction data that does not match any individual that was used in the model estimation. Here, we will use ID values `"male_notrt"`, `"female_notrt"`, `"male_trt"`, and `"female_trt"` for our four new individuals in our prediction data. Each individual will have a different combination of `sex` and `trt`, and for the time variable `year` the prediction data will contain values from 0 through to 10.
...
Just to reiterate; because the individuals with an ID `"male_notrt"`, `"female_notrt"`, etc, were not used to estimate the model, the predictions will be marginalised over the distribution of the individual-specific parameters. We could have given the individuals in the prediction data any ID value, just as long as that ID value didn't match an individual who was included in the estimation sample. 

To predict the marginal longitudinal trajectory for log serum bilirubin under each covariate profile and plot it we can type:

```{r plot_traj_218391, eval=FALSE}
p1 <- posterior_traj(mod3, m = 1, newdata = ndL)
plot(p1) + ggplot2::coord_cartesian(ylim = c(-10,15))
```

Because we are marginalising over the distribution of the individual-level random effects, we are incorporating all the variation related to between-individual differences, and therefore the prediction interval is wide (shown by the shaded area around the marginal longitudinal trajectory). The magnitude of the effects of both `sex` and `trt` are relatively small compared to the population-level effect of `year` and the between-individual variation (i.e. the individual-level random effects) which we are marginalising over. For example, here are the point estimates for the population-level effects of `sex`, `trt`, and `year`:
...
This shows us that the point estimates for the population-level effects of `sex` and `trt` are 0.570 and -0.097, respectively, whereas the standard deviation for the individual-level random intercepts are 1.24; hence, the width of the uncertainty intervals for the marginal longitudinal trajectories (i.e. the grey shaded area in the plots) which incorporate variation due to the individual-level random effects swamp any effects of gender and treatment.
