---
title: "Estimating ANOVA Models with the rstanarm Package"
...
%\VignetteIndexEntry{Estimating ANOVA Models with the rstanarm Package}
-->

# Introduction

This vignette explains how to estimate ANalysis Of VAriance (ANOVA) models 
using the __rstanarm__ package.
...
the tautology $1 = R^2 + \left(1 - R^2\right)$ but it is reasonable to expect
a researcher to have a plausible guess for $R^2$ before conducting an ANOVA.
See the vignette entitled "Estimating Linear Models with the __rstanarm__ 
Package" for more information on this approach.
...
the vignette entitled "Estimating __lme4__-style Models with the __rstanarm__
Pakcage" for more information on this approach.
...
a moderate proportion of the variance in weightgain might be attributed to
...
post1 <- stan_aov(weightgain ~ source * type, data = weightgain, 
                  prior = R2(location = 0.5), seed = 12345, cores = 2,
                  control = list(adapt_delta = 0.999, max_treedepth = 13))
post1
```
Here we have specified the optional `control` argument as a list with elements
`adapt_delta = 0.999` to decrease the stepsize and largely prevent divergent
transitions and `max_treedepth = 12` in order to allow the numerical integrator
to take more steps than the default value. The results suggest that only the
treatment worth writing home about is whether the rats were fed a low protein
or high protein diet. Also, our prior guess that $R^2 = 0.5$ was overly 
optimistic, and the model overfits the data a little bit. However, the 
frequentist estimates presumably overfit the data even more.
...
post2 <- stan_lmer(weightgain ~ 1 + (1|source) + (1|type) + (1|source:type),
                   data = weightgain, prior.for.intercept = cauchy(),
                   prior.for.covariance = decov(gamma_shape = 2, scale = 2),
                   seed = 12345, cores = 2, 
