...
save(state_plot_data, file = "mrp-files/state_plot_data.rda")
...
```{r,echo=FALSE, message=FALSE, warning=FALSE, results='hide', cache = cachemodel}
fit <- stan_glmer(
...
model_popn_pref <- c(mean = round(mean(poststrat_prob), 3), sd = round(sd(poststrat_prob), 3))
...
sample_popn_pref <- round(mean(sample$cat_pref), 3)
...
true_popn_pref <-
  round(sum(true_popn$cat_pref * poststrat$N) / sum(poststrat$N), 3)
...
```{r} 
c(
  mean = round(mean(abs(state_df$sample_state_pref-state_df$true_state_pref),
                    na.rm = TRUE)*100),
  max = round(max(abs(state_df$sample_state_pref-state_df$true_state_pref), 
                  na.rm = TRUE)*100)
)
```
...
model_popn_pref_alt <- c(mean = round(mean(poststrat_prob_alt), 3), 
                         sd = round(sd(poststrat_prob_alt), 3))
...
[here](https://cran.r-project.org/web/packages/lme4/vignettes/lmer.pdf).
