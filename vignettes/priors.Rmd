...
### Default prior scale adjustments
...
**rstanarm** used to require you to also explicitly set the `autoscale` argument
to `FALSE`, but now autoscaling only happens for the default priors, and
manually specified priors do not autoscale by default unless `autoscale` is set
to `TRUE`. For example:
