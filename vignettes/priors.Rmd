...
list), if `autoscale` is set to `TRUE` then, in certain cases,
...
**rstanarm** used to require you to also explicitly set the `autoscale` argument to to `FALSE`, but now autoscaling only happens for the default priors and manually
specified priors do not autoscale by default unless `autoscale` is set to `TRUE`. For example:
