...
is important to first go over the [How to Use the __rstanarm__
Package](rstanarm.html) vignette, which provides a general overview of the
package.
...
functions also explain which distributions can be used when specifying 
...
The `stan_glm` function accepts the arguments `prior_intercept`, `prior`, 
and `prior_aux` arguments but here we will leave them at their defaults.
...
deviation, has a default prior that is $\mathsf{halfCauchy}(0, 5)$. However, in
order for the default to be weakly informative __rstanarm__ will adjust the 
scales of these priors. As a result, the actual scale used was `r 
fr2(priors$prior_aux$adjusted_scale)`. The default priors for the regression 
coefficients (in this case the coefficients on the `wt` and `am` variables) are 
independent normal distributions with mean 0 and scale (standard deviation) 
$2.5$. Like for `sigma`, in order for the default to be weakly informative 
__rstanarm__ will adjust the scales of the priors on the coefficients. As a 
result, the prior scales actually used were
`r fr2(priors$prior$adjusted_scale[1])` and 
`r fr2(priors$prior$adjusted_scale[2])`. 
For the intercept we can see that the default prior is normal with mean 0 and
standard deviation $10$, but in this case the standard deviation was adjusted to
`r fr2(priors$prior_intercept$adjusted_scale[1])`. 
The next two subsections go into more detail about how this
rescaling works and how to easily disable it if desired.
