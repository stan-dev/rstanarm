  ...
  }
  {
    ...
    if (family[m] == 1) {  // gaussian
	  for (n in 1:NM[m]) mean_PPD[m] = mean_PPD[m] + normal_rng(eta_tmp[n], aux[aux_mark]);
    }
	else if (family[m] == 2) {  // gamma
	  for (n in 1:NM[m]) mean_PPD[m] = mean_PPD[m] + gamma_rng(aux[aux_mark], aux[aux_mark] / eta_tmp[n]);
    }
    else if (family[m] == 3) {  // inverse gaussian
      for (n in 1:NM[m]) mean_PPD[m] = mean_PPD[m] + inv_gaussian_rng(eta_tmp[n], aux[aux_mark]);
    }
     else if (family[m] == 4) {  // bernoulli
      for (n in 1:NM[m]) mean_PPD[m] = mean_PPD[m] + bernoulli_rng(eta_tmp[n]);
    } 
    else if (family[m] == 5) {  // binomial
	  int trials_tmp[NM[m]];
	  trials_tmp = trials[idx[m,1]:idx[m,2]];
      for (n in 1:NM[m]) mean_PPD[m] = mean_PPD[m] + binomial_rng(trials_tmp[n], eta_tmp[n]);
    }
    else if (family[m] == 6 || family[m] == 8) for (n in 1:NM[m]) {  // poisson or poisson-gamma
