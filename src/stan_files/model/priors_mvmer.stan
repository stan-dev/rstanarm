  ...
  if (M > 0 && has_aux[1] == 1) {
    real dummy = aux_lp(yAux1_unscaled[1], y_prior_dist_for_aux[1],
                        y_prior_scale_for_aux[1], y_prior_df_for_aux[1]);
  }
  if (M > 1 && has_aux[2] == 1) {
    real dummy = aux_lp(yAux2_unscaled[1], y_prior_dist_for_aux[2],
                        y_prior_scale_for_aux[2], y_prior_df_for_aux[2]);
  }
  if (M > 2 && has_aux[3] == 1) {
    real dummy = aux_lp(yAux3_unscaled[1], y_prior_dist_for_aux[3],
                        y_prior_scale_for_aux[3], y_prior_df_for_aux[3]);
  }

  // Log priors, intercepts
  if (M > 0 && intercept_type[1] > 0) {
    real dummy = gamma_lp(yGamma1[1],
                          y_prior_dist_for_intercept[1],
                          y_prior_mean_for_intercept[1],
                          y_prior_scale_for_intercept[1],
                          y_prior_df_for_intercept[1]);
  }
  if (M > 1 && intercept_type[2] > 0) {
    real dummy = gamma_lp(yGamma2[1],
                          y_prior_dist_for_intercept[2],
                          y_prior_mean_for_intercept[2],
                          y_prior_scale_for_intercept[2],
                          y_prior_df_for_intercept[2]);
  }
  if (M > 2 && intercept_type[3] > 0) {
    real dummy = gamma_lp(yGamma3[1],
                          y_prior_dist_for_intercept[3],
                          y_prior_mean_for_intercept[3],
                          y_prior_scale_for_intercept[3],
                          y_prior_df_for_intercept[3]);
  }

  // Log priors, population level params
  if (M > 0 && yK[1] > 0) {
    real dummy = beta_lp(z_yBeta1, y_prior_dist[1], y_prior_scale1, y_prior_df1,
                         y_global_prior_df[1], yLocal1, yGlobal1, yMix1, yOol1,
                         y_slab_df[1], y_caux1);
  }
  if (M > 1 && yK[2] > 0) {
    real dummy = beta_lp(z_yBeta2, y_prior_dist[2], y_prior_scale2, y_prior_df2,
                         y_global_prior_df[2], yLocal2, yGlobal2, yMix2, yOol2,
                         y_slab_df[2], y_caux2);
  }
  if (M > 2 && yK[3] > 0) {
    real dummy = beta_lp(z_yBeta3, y_prior_dist[3], y_prior_scale3, y_prior_df3,
