...
stan_nlmer <-
  function(
    ...
    Z <- pad_reTrms(
      Ztlist = nlf$reTrms$Ztlist,
      cnms = nlf$reTrms$cnms,
      flist = nlf$reTrms$flist
    )$Z
...
make_nlf_family <- function(SSfun_char, nlf) {
  ...
  g$linkinv <- function(eta, arg1, arg2 = NULL, FUN = SSfun_char) {
    if (is.matrix(eta)) {
      ...
      if (is.null(arg2)) {
        SSargs <- c(list(arg1), SSargs)
      } else {
        SSargs <- c(list(arg1, arg2), SSargs)
      }
    } else {
      SSargs <- as.data.frame(matrix(eta, nrow = length(arg1)))
      if (is.null(arg2)) {
        SSargs <- cbind(arg1, SSargs)
      } else {
        SSargs <- cbind(arg1, arg2, SSargs)
      }
    }
