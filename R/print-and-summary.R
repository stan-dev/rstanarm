...
# still need to do  R2 prior, etc.
print_prior_summary <- function(priors, digits = 2) {
  ...
  coef_adj_scale <- if (is.null(prior_coef))
    NULL else prior_coef$adjusted_scale
  int_adj_scale <- if (is.null(prior_intercept))
    NULL else prior_intercept$adjusted_scale
  any_adjusted_scales <- !is.null(coef_adj_scale) || !is.null(int_adj_scale)
  if (any_adjusted_scales)
    cat("\n User-specified:")
    ...
    if (coef_dist %in% c("normal", "student_t", "cauchy")) {
      ...
      prior_coef$scale <-
        paste0("[", paste(c((prior_coef$scale[1:2]), if (k > 2)
          "..."), collapse = ","), "]")
      if (!is.null(coef_adj_scale))
        coef_adj_scale <-
          paste0("[", paste(c(.fr2(coef_adj_scale[1:2]), if (k > 2)
            "..."), collapse = ","), "]")
  ...
  if (!is.null(priors$prior_covariance)) {
    j <- length(priors$prior_covariance$regularization)
    if (j >= 2) {
      priors <- within(priors, {
  ...
  if (any_adjusted_scales) {
    cat("\n Adjusted prior scale(s):")
    if (!is.null(int_adj_scale))
      cat("\n  intercept: scale =", .fr3(int_adj_scale))
    if (!is.null(coef_adj_scale))
      cat("\n  coefficients: scale =", .fr3(coef_adj_scale))
  }
}
