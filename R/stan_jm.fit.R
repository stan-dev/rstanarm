...
stan_jm.fit <- function(formulaLong = NULL, dataLong = NULL, formulaEvent = NULL, 
  ...
  standata$yNeta <- 
    fetch_array(y_mod, "x", "N", pad_length = 20) # same as Nobs for stan_mvmer
  standata$yK <- 
  ...
  standata$yInt4 <- if (M > 3) Y_integer[[4]] else as.array(integer(0))
  standata$yInt5 <- if (M > 4) Y_integer[[5]] else as.array(integer(0))
  standata$yInt6 <- if (M > 5) Y_integer[[6]] else as.array(integer(0))
  standata$yInt7 <- if (M > 6) Y_integer[[7]] else as.array(integer(0))
  standata$yInt8 <- if (M > 7) Y_integer[[8]] else as.array(integer(0))
  standata$yInt9 <- if (M > 8) Y_integer[[9]] else as.array(integer(0))
  standata$yInt10 <- if (M > 9) Y_integer[[10]] else as.array(integer(0))
  standata$yInt11 <- if (M > 10) Y_integer[[11]] else as.array(integer(0))
  standata$yInt12 <- if (M > 11) Y_integer[[12]] else as.array(integer(0))
  standata$yInt13 <- if (M > 12) Y_integer[[13]] else as.array(integer(0))
  standata$yInt14 <- if (M > 13) Y_integer[[14]] else as.array(integer(0))
  standata$yInt15 <- if (M > 14) Y_integer[[15]] else as.array(integer(0))
  standata$yInt16 <- if (M > 15) Y_integer[[16]] else as.array(integer(0))
  standata$yInt17 <- if (M > 16) Y_integer[[17]] else as.array(integer(0))
  standata$yInt18 <- if (M > 17) Y_integer[[18]] else as.array(integer(0))
  standata$yInt19 <- if (M > 18) Y_integer[[19]] else as.array(integer(0))
  standata$yInt20 <- if (M > 19) Y_integer[[20]] else as.array(integer(0))
  ...
  standata$yReal4 <- if (M > 3) Y_real[[4]] else as.array(double(0)) 
  standata$yReal5 <- if (M > 4) Y_real[[5]] else as.array(double(0)) 
  standata$yReal6 <- if (M > 5) Y_real[[6]] else as.array(double(0)) 
  standata$yReal7 <- if (M > 6) Y_real[[7]] else as.array(double(0)) 
  standata$yReal8 <- if (M > 7) Y_real[[8]] else as.array(double(0)) 
  standata$yReal9 <- if (M > 8) Y_real[[9]] else as.array(double(0)) 
  standata$yReal10 <- if (M > 9) Y_real[[10]] else as.array(double(0)) 
  standata$yReal11 <- if (M > 10) Y_real[[11]] else as.array(double(0)) 
  standata$yReal12 <- if (M > 11) Y_real[[12]] else as.array(double(0)) 
  standata$yReal13 <- if (M > 12) Y_real[[13]] else as.array(double(0)) 
  standata$yReal14 <- if (M > 13) Y_real[[14]] else as.array(double(0)) 
  standata$yReal15 <- if (M > 14) Y_real[[15]] else as.array(double(0)) 
  standata$yReal16 <- if (M > 15) Y_real[[16]] else as.array(double(0)) 
  standata$yReal17 <- if (M > 16) Y_real[[17]] else as.array(double(0)) 
  standata$yReal18 <- if (M > 17) Y_real[[18]] else as.array(double(0)) 
  standata$yReal19 <- if (M > 18) Y_real[[19]] else as.array(double(0)) 
  standata$yReal20 <- if (M > 19) Y_real[[20]] else as.array(double(0)) 
  ...
  standata$yX4 <- if (M > 3) X[[4]] else matrix(0,0,0)
  standata$yX5 <- if (M > 4) X[[5]] else matrix(0,0,0)
  standata$yX6 <- if (M > 5) X[[6]] else matrix(0,0,0)
  standata$yX7 <- if (M > 6) X[[7]] else matrix(0,0,0)
  standata$yX8 <- if (M > 7) X[[8]] else matrix(0,0,0)
  standata$yX9 <- if (M > 8) X[[9]] else matrix(0,0,0)
  standata$yX10 <- if (M > 9) X[[10]] else matrix(0,0,0)
  standata$yX11 <- if (M > 10) X[[11]] else matrix(0,0,0)
  standata$yX12 <- if (M > 11) X[[12]] else matrix(0,0,0)
  standata$yX13 <- if (M > 12) X[[13]] else matrix(0,0,0)
  standata$yX14 <- if (M > 13) X[[14]] else matrix(0,0,0)
  standata$yX15 <- if (M > 14) X[[15]] else matrix(0,0,0)
  standata$yX16 <- if (M > 15) X[[16]] else matrix(0,0,0)
  standata$yX17 <- if (M > 16) X[[17]] else matrix(0,0,0)
  standata$yX18 <- if (M > 17) X[[18]] else matrix(0,0,0)
  standata$yX19 <- if (M > 18) X[[19]] else matrix(0,0,0)
  standata$yX20 <- if (M > 19) X[[20]] else matrix(0,0,0)
  ...
  standata$yXbar4 <- if (M > 3) as.array(X_bar[[4]]) else as.array(double(0))
  standata$yXbar5 <- if (M > 4) as.array(X_bar[[5]]) else as.array(double(0))
  standata$yXbar6 <- if (M > 5) as.array(X_bar[[6]]) else as.array(double(0))
  standata$yXbar7 <- if (M > 6) as.array(X_bar[[7]]) else as.array(double(0))
  standata$yXbar8 <- if (M > 7) as.array(X_bar[[8]]) else as.array(double(0))
  standata$yXbar9 <- if (M > 8) as.array(X_bar[[9]]) else as.array(double(0))
  standata$yXbar10 <- if (M > 9) as.array(X_bar[[10]]) else as.array(double(0))
  standata$yXbar11 <- if (M > 10) as.array(X_bar[[11]]) else as.array(double(0))
  standata$yXbar12 <- if (M > 11) as.array(X_bar[[12]]) else as.array(double(0))
  standata$yXbar13 <- if (M > 12) as.array(X_bar[[13]]) else as.array(double(0))
  standata$yXbar14 <- if (M > 13) as.array(X_bar[[14]]) else as.array(double(0))
  standata$yXbar15 <- if (M > 14) as.array(X_bar[[15]]) else as.array(double(0))
  standata$yXbar16 <- if (M > 15) as.array(X_bar[[16]]) else as.array(double(0))
  standata$yXbar17 <- if (M > 16) as.array(X_bar[[17]]) else as.array(double(0))
  standata$yXbar18 <- if (M > 17) as.array(X_bar[[18]]) else as.array(double(0))
  standata$yXbar19 <- if (M > 18) as.array(X_bar[[19]]) else as.array(double(0))
  standata$yXbar20 <- if (M > 19) as.array(X_bar[[20]]) else as.array(double(0))
  ...
  standata$y4_Z1 <- if (M > 3) Z1[[4L]] else matrix(0,0,0)
  standata$y5_Z1 <- if (M > 4) Z1[[5L]] else matrix(0,0,0)
  standata$y6_Z1 <- if (M > 5) Z1[[6L]] else matrix(0,0,0)
  standata$y7_Z1 <- if (M > 6) Z1[[7L]] else matrix(0,0,0)
  standata$y8_Z1 <- if (M > 7) Z1[[8L]] else matrix(0,0,0)
  standata$y9_Z1 <- if (M > 8) Z1[[9L]] else matrix(0,0,0)
  standata$y10_Z1 <- if (M > 9) Z1[[10L]] else matrix(0,0,0)
  standata$y11_Z1 <- if (M > 10) Z1[[11L]] else matrix(0,0,0)
  standata$y12_Z1 <- if (M > 11) Z1[[12L]] else matrix(0,0,0)
  standata$y13_Z1 <- if (M > 12) Z1[[13L]] else matrix(0,0,0)
  standata$y14_Z1 <- if (M > 13) Z1[[14L]] else matrix(0,0,0)
  standata$y15_Z1 <- if (M > 14) Z1[[15L]] else matrix(0,0,0)
  standata$y16_Z1 <- if (M > 15) Z1[[16L]] else matrix(0,0,0)
  standata$y17_Z1 <- if (M > 16) Z1[[17L]] else matrix(0,0,0)
  standata$y18_Z1 <- if (M > 17) Z1[[18L]] else matrix(0,0,0)
  standata$y19_Z1 <- if (M > 18) Z1[[19L]] else matrix(0,0,0)
  standata$y20_Z1 <- if (M > 19) Z1[[20L]] else matrix(0,0,0)
  ...
  standata$y4_Z1_id <- if (M > 3) Z1_id[[4L]] else as.array(integer(0))
  standata$y5_Z1_id <- if (M > 4) Z1_id[[5L]] else as.array(integer(0))
  standata$y6_Z1_id <- if (M > 5) Z1_id[[6L]] else as.array(integer(0))
  standata$y7_Z1_id <- if (M > 6) Z1_id[[7L]] else as.array(integer(0))
  standata$y8_Z1_id <- if (M > 7) Z1_id[[8L]] else as.array(integer(0))
  standata$y9_Z1_id <- if (M > 8) Z1_id[[9L]] else as.array(integer(0))
  standata$y10_Z1_id <- if (M > 9) Z1_id[[10L]] else as.array(integer(0))
  standata$y11_Z1_id <- if (M > 10) Z1_id[[11L]] else as.array(integer(0))
  standata$y12_Z1_id <- if (M > 11) Z1_id[[12L]] else as.array(integer(0))
  standata$y13_Z1_id <- if (M > 12) Z1_id[[13L]] else as.array(integer(0))
  standata$y14_Z1_id <- if (M > 13) Z1_id[[14L]] else as.array(integer(0))
  standata$y15_Z1_id <- if (M > 14) Z1_id[[15L]] else as.array(integer(0))
  standata$y16_Z1_id <- if (M > 15) Z1_id[[16L]] else as.array(integer(0))
  standata$y17_Z1_id <- if (M > 16) Z1_id[[17L]] else as.array(integer(0))
  standata$y18_Z1_id <- if (M > 17) Z1_id[[18L]] else as.array(integer(0))
  standata$y19_Z1_id <- if (M > 18) Z1_id[[19L]] else as.array(integer(0))
  standata$y20_Z1_id <- if (M > 19) Z1_id[[20L]] else as.array(integer(0))
  ...
  if (length(cnms) > 1L) {
    ...
    standata$y4_Z2 <- if (M > 3) Z3[[4L]] else matrix(0,0,0)
    standata$y5_Z2 <- if (M > 4) Z4[[5L]] else matrix(0,0,0)
    standata$y6_Z2 <- if (M > 5) Z5[[6L]] else matrix(0,0,0)
    standata$y7_Z2 <- if (M > 6) Z6[[7L]] else matrix(0,0,0)
    standata$y8_Z2 <- if (M > 7) Z7[[8L]] else matrix(0,0,0)
    standata$y9_Z2 <- if (M > 8) Z8[[9L]] else matrix(0,0,0)
    standata$y10_Z2 <- if (M > 9) Z9[[10L]] else matrix(0,0,0)
    standata$y11_Z2 <- if (M > 10) Z10[[11L]] else matrix(0,0,0)
    standata$y12_Z2 <- if (M > 11) Z11[[12L]] else matrix(0,0,0)
    standata$y13_Z2 <- if (M > 12) Z12[[13L]] else matrix(0,0,0)
    standata$y14_Z2 <- if (M > 13) Z13[[14L]] else matrix(0,0,0)
    standata$y15_Z2 <- if (M > 14) Z14[[15L]] else matrix(0,0,0)
    standata$y16_Z2 <- if (M > 15) Z15[[16L]] else matrix(0,0,0)
    standata$y17_Z2 <- if (M > 16) Z16[[17L]] else matrix(0,0,0)
    standata$y18_Z2 <- if (M > 17) Z17[[18L]] else matrix(0,0,0)
    standata$y19_Z2 <- if (M > 18) Z18[[19L]] else matrix(0,0,0)
    standata$y20_Z2 <- if (M > 19) Z19[[20L]] else matrix(0,0,0)
    ...
    standata$y4_Z2_id <- if (M > 3) Z2_id[[4L]] else as.array(integer(0))
    standata$y5_Z2_id <- if (M > 4) Z2_id[[5L]] else as.array(integer(0))
    standata$y6_Z2_id <- if (M > 5) Z2_id[[6L]] else as.array(integer(0))
    standata$y7_Z2_id <- if (M > 6) Z2_id[[7L]] else as.array(integer(0))
    standata$y8_Z2_id <- if (M > 7) Z2_id[[8L]] else as.array(integer(0))
    standata$y9_Z2_id <- if (M > 8) Z2_id[[9L]] else as.array(integer(0))
    standata$y10_Z2_id <- if (M > 9) Z2_id[[10L]] else as.array(integer(0))
    standata$y11_Z2_id <- if (M > 10) Z2_id[[11L]] else as.array(integer(0))
    standata$y12_Z2_id <- if (M > 11) Z2_id[[12L]] else as.array(integer(0))
    standata$y13_Z2_id <- if (M > 12) Z2_id[[13L]] else as.array(integer(0))
    standata$y14_Z2_id <- if (M > 13) Z2_id[[14L]] else as.array(integer(0))
    standata$y15_Z2_id <- if (M > 14) Z2_id[[15L]] else as.array(integer(0))
    standata$y16_Z2_id <- if (M > 15) Z2_id[[16L]] else as.array(integer(0))
    standata$y17_Z2_id <- if (M > 16) Z2_id[[17L]] else as.array(integer(0))
    standata$y18_Z2_id <- if (M > 17) Z2_id[[18L]] else as.array(integer(0))
    standata$y19_Z2_id <- if (M > 18) Z2_id[[19L]] else as.array(integer(0))
    standata$y20_Z2_id <- if (M > 19) Z2_id[[20L]] else as.array(integer(0))
  } else {
    ...
    standata$y4_Z2 <- matrix(0,0,0)
    standata$y5_Z2 <- matrix(0,0,0)
    standata$y6_Z2 <- matrix(0,0,0)
    standata$y7_Z2 <- matrix(0,0,0)
    standata$y8_Z2 <- matrix(0,0,0)
    standata$y9_Z2 <- matrix(0,0,0)
    standata$y10_Z2 <- matrix(0,0,0)
    standata$y11_Z2 <- matrix(0,0,0)
    standata$y12_Z2 <- matrix(0,0,0)
    standata$y13_Z2 <- matrix(0,0,0)
    standata$y14_Z2 <- matrix(0,0,0)
    standata$y15_Z2 <- matrix(0,0,0)
    standata$y16_Z2 <- matrix(0,0,0)
    standata$y17_Z2 <- matrix(0,0,0)
    standata$y18_Z2 <- matrix(0,0,0)
    standata$y19_Z2 <- matrix(0,0,0)
    standata$y20_Z2 <- matrix(0,0,0)
    standata$y1_Z2_id <- as.array(integer(0))
    standata$y2_Z2_id <- as.array(integer(0))
    standata$y3_Z2_id <- as.array(integer(0))
    standata$y4_Z2_id <- as.array(integer(0))
    standata$y5_Z2_id <- as.array(integer(0))
    standata$y6_Z2_id <- as.array(integer(0))
    standata$y7_Z2_id <- as.array(integer(0))
    standata$y8_Z2_id <- as.array(integer(0))
    standata$y9_Z2_id <- as.array(integer(0))
    standata$y10_Z2_id <- as.array(integer(0))
    standata$y11_Z2_id <- as.array(integer(0))
    standata$y12_Z2_id <- as.array(integer(0))
    standata$y13_Z2_id <- as.array(integer(0))
    standata$y14_Z2_id <- as.array(integer(0))
    standata$y15_Z2_id <- as.array(integer(0))
    standata$y16_Z2_id <- as.array(integer(0))
    standata$y17_Z2_id <- as.array(integer(0))
    standata$y18_Z2_id <- as.array(integer(0))
    standata$y19_Z2_id <- as.array(integer(0))
    standata$y20_Z2_id <- as.array(integer(0))
  }
  ...
  standata$y_prior_mean4 <- if (M > 3) prior_mean[[4]] else as.array(double(0))
  standata$y_prior_mean5 <- if (M > 4) prior_mean[[5]] else as.array(double(0))
  standata$y_prior_mean6 <- if (M > 5) prior_mean[[6]] else as.array(double(0))
  standata$y_prior_mean7 <- if (M > 6) prior_mean[[7]] else as.array(double(0))
  standata$y_prior_mean8 <- if (M > 7) prior_mean[[8]] else as.array(double(0))
  standata$y_prior_mean9 <- if (M > 8) prior_mean[[9]] else as.array(double(0))
  standata$y_prior_mean10 <- if (M > 9) prior_mean[[10]] else as.array(double(0))
  standata$y_prior_mean11 <- if (M > 10) prior_mean[[11]] else as.array(double(0))
  standata$y_prior_mean12 <- if (M > 11) prior_mean[[12]] else as.array(double(0))
  standata$y_prior_mean13 <- if (M > 12) prior_mean[[13]] else as.array(double(0))
  standata$y_prior_mean14 <- if (M > 13) prior_mean[[14]] else as.array(double(0))
  standata$y_prior_mean15 <- if (M > 14) prior_mean[[15]] else as.array(double(0))
  standata$y_prior_mean16 <- if (M > 15) prior_mean[[16]] else as.array(double(0))
  standata$y_prior_mean17 <- if (M > 16) prior_mean[[17]] else as.array(double(0))
  standata$y_prior_mean18 <- if (M > 17) prior_mean[[18]] else as.array(double(0))
  standata$y_prior_mean19 <- if (M > 18) prior_mean[[19]] else as.array(double(0))
  standata$y_prior_mean20 <- if (M > 19) prior_mean[[20]] else as.array(double(0))
  ...
  standata$y_prior_scale4 <- if (M > 3) as.array(prior_scale[[4]]) else as.array(double(0))
  standata$y_prior_scale5 <- if (M > 4) as.array(prior_scale[[5]]) else as.array(double(0))
  standata$y_prior_scale6 <- if (M > 5) as.array(prior_scale[[6]]) else as.array(double(0))
  standata$y_prior_scale7 <- if (M > 6) as.array(prior_scale[[7]]) else as.array(double(0))
  standata$y_prior_scale8 <- if (M > 7) as.array(prior_scale[[8]]) else as.array(double(0))
  standata$y_prior_scale9 <- if (M > 8) as.array(prior_scale[[9]]) else as.array(double(0))
  standata$y_prior_scale10 <- if (M > 9) as.array(prior_scale[[10]]) else as.array(double(0))
  standata$y_prior_scale11 <- if (M > 10) as.array(prior_scale[[11]]) else as.array(double(0))
  standata$y_prior_scale12 <- if (M > 11) as.array(prior_scale[[12]]) else as.array(double(0))
  standata$y_prior_scale13 <- if (M > 12) as.array(prior_scale[[13]]) else as.array(double(0))
  standata$y_prior_scale14 <- if (M > 13) as.array(prior_scale[[14]]) else as.array(double(0))
  standata$y_prior_scale15 <- if (M > 14) as.array(prior_scale[[15]]) else as.array(double(0))
  standata$y_prior_scale16 <- if (M > 15) as.array(prior_scale[[16]]) else as.array(double(0))
  standata$y_prior_scale17 <- if (M > 16) as.array(prior_scale[[17]]) else as.array(double(0))
  standata$y_prior_scale18 <- if (M > 17) as.array(prior_scale[[18]]) else as.array(double(0))
  standata$y_prior_scale19 <- if (M > 18) as.array(prior_scale[[19]]) else as.array(double(0))
  standata$y_prior_scale20 <- if (M > 19) as.array(prior_scale[[20]]) else as.array(double(0))
  ...
  standata$y_prior_df4 <- if (M > 3) prior_df[[4]] else as.array(double(0))
  standata$y_prior_df5 <- if (M > 4) prior_df[[5]] else as.array(double(0))
  standata$y_prior_df6 <- if (M > 5) prior_df[[6]] else as.array(double(0))
  standata$y_prior_df7 <- if (M > 6) prior_df[[7]] else as.array(double(0))
  standata$y_prior_df8 <- if (M > 7) prior_df[[8]] else as.array(double(0))
  standata$y_prior_df9 <- if (M > 8) prior_df[[9]] else as.array(double(0))
  standata$y_prior_df10 <- if (M > 9) prior_df[[10]] else as.array(double(0))
  standata$y_prior_df11 <- if (M > 10) prior_df[[11]] else as.array(double(0))
  standata$y_prior_df12 <- if (M > 11) prior_df[[12]] else as.array(double(0))
  standata$y_prior_df13 <- if (M > 12) prior_df[[13]] else as.array(double(0))
  standata$y_prior_df14 <- if (M > 13) prior_df[[14]] else as.array(double(0))
  standata$y_prior_df15 <- if (M > 14) prior_df[[15]] else as.array(double(0))
  standata$y_prior_df16 <- if (M > 15) prior_df[[16]] else as.array(double(0))
  standata$y_prior_df17 <- if (M > 16) prior_df[[17]] else as.array(double(0))
  standata$y_prior_df18 <- if (M > 17) prior_df[[18]] else as.array(double(0))
  standata$y_prior_df19 <- if (M > 18) prior_df[[19]] else as.array(double(0))
  standata$y_prior_df20 <- if (M > 19) prior_df[[20]] else as.array(double(0))
