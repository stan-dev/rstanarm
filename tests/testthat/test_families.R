...
test_that("rstanarm_family works", {
  ...
  expect_equal(gaus1$hyperparams$prior_scale_for_dispersion, 2)
  expect_false(gaus1$hyperparams$scaled)
  ...
  expect_equal(gamma1$hyperparams$prior_scale_for_dispersion, 3)
  expect_equal(gamma1$hyperparams$min_prior_scale, 0.1)
  expect_false(gamma1$hyperparams$scaled)
  
  gamma3 <- rstanarm_family("Gamma")
  expect_equal(gamma3$family, Gamma())
  expect_equal(gamma3$hyperparams$prior_scale_for_dispersion, 5)
  expect_equal(gamma3$hyperparams$min_prior_scale, 1e-12)
  expect_true(gamma3$hyperparams$scaled)
  
  t1 <- rstanarm_family(family = "t_family")
  expect_true(is.rstanarm_family(t1))
  expect_equal(t1$family, t_family())
  expect_equal(t1$hyperparams$prior_scale_for_dispersion, 5)
  expect_equal(t1$hyperparams$prior_shape_for_df, 2)
  expect_equal(t1$hyperparams$prior_rate_for_df, 0.1)
  expect_true(t1$hyperparams$scaled)
  ...
  expect_equal(t2$hyperparams$prior_scale_for_dispersion, 5)
  expect_equal(t2$hyperparams$prior_shape_for_df, 2)
  expect_equal(t2$hyperparams$prior_rate_for_df, NULL)
  expect_false(t2$hyperparams$scaled)
})
