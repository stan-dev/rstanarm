...
if (.Platform$OS.type != "windows") {
  ...
  functions <- sapply(dir(system.file("stan", package = "rstanarm"),
                      pattern = "stan$", full.names = TRUE, recursive = TRUE), function(f) {
    mc <- readLines(f)
  ...
  functions <- c(unlist(lapply(file.path(system.file("stan", package = "rstanarm"), "functions",
