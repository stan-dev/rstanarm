...
test_that("predict ok for binomial", {
  ...
  expect_equal(pg$fit, ps$fit, tolerance = 0.1)
  expect_equal(pg$fit, pso$fit, tolerance = 0.05)
  expect_equal(pg$se.fit, ps$se.fit, tolerance = 0.2)
  expect_equal(pg$se.fit, pso$se.fit, tolerance = 0.1)
  expect_equal(presp(glmfit)[1:2], presp(stanfit_opt), tolerance = 0.05)
  ...
  expect_equal(pg$se.fit, ps$se.fit, tolerance = 0.2)
  expect_equal(pg$se.fit, pso$se.fit, tolerance = 0.1)
  expect_equal(presp(glmfit, newd)[1:2], presp(stanfit_opt, newd), tolerance = 0.1)
})
...
test_that("predict ok for gaussian", {
  ...
  ps <- plink(stanfit)
  pso <- plink(stanfit_opt)
  expect_equal(pg$fit, ps$fit, tolerance = 0.05)
  expect_equal(pg$fit, pso$fit, tolerance = 0.05)
  expect_equal(pg$se.fit, ps$se.fit, tolerance = 0.3)
  expect_equal(pg$se.fit, pso$se.fit, tolerance = 0.1)
  expect_equal(presp(glmfit)[1:2], presp(stanfit_opt), tolerance = 0.1)
...
test_that("predict ok for Poisson", {
  ...
  expect_equal(pg$fit, ps$fit, tolerance = 0.05)
  expect_equal(pg$fit, pso$fit, tolerance = 0.05)
  expect_equal(pg$se.fit, ps$se.fit, tolerance = 0.1)
  expect_equal(pg$se.fit, pso$se.fit, tolerance = 0.1)
  expect_equal(presp(glmfit)[1:2], presp(stanfit_opt), tolerance = 0.1)

  expect_equal(plink(stanfit, sef = FALSE), plink(glmfit, sef = FALSE), tolerance = 0.05)
  expect_equal(presp(stanfit, sef = FALSE), presp(glmfit, sef = FALSE), tolerance = 0.05)
